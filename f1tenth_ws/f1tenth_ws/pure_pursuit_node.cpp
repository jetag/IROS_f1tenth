...
private:
    ...
    double look_ahead, kp, vel_min, vel_max, vel_inter, velocity, ttc;
    double pure_x,pure_y, base_x, base_y, yaw, steering_angle, dist_to_pure, scale;
    bool drive_flag;
...
public:
    ...
    void publish_gap_follow(std::vector<float> ranges){
        ...
        for (int i=0;i<ranges_processed.size();i++){
            if (ranges_processed[i] == max_gap_dist){
                ...
                if (gap > max_gap){
                    max_gap = gap;
                    max_gap_idx = i;
                    buffer_angle = buffer/ranges_processed[i];
                }
                gap = 0;
            }
        }
        ...
        float angle_max = 2.3499;
        float angle = deg_to_rad(mid_idx/4)-angle_max;
    ...
    void check_pure_pursuit(){
        ...
        velocity = waypoints[min_idx][2]*scale;
        //pure to base dist
        dist_to_pure = sqrt((pure_x-base_x)*(pure_x-base_x) + (pure_y-base_y)*(pure_y-base_y));
